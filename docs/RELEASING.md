...
This project must not be released from a workstation that contains a live Bot Token unless the release contents have been independently checked.
...
4. Inspect the generated LaunchAgent definition with `python3 -c 'import sms_bridge; print(sms_bridge.launch_agent_plist())'`. It must not contain a Bot Token, chat ID, pairing URL, or OTP. Confirm logs point to `~/Library/Application Support/SMS Bridge`, not the source tree.
...
如果开发工作站中存在仍在使用的 Bot Token，未经独立检查不得直接从该工作站发布。
...
4. 用 `python3 -c 'import sms_bridge; print(sms_bridge.launch_agent_plist())'` 检查生成的 LaunchAgent。它不得包含 Bot Token、Chat ID、配对 URL 或验证码；日志路径应位于 `~/Library/Application Support/SMS Bridge`，而不是源码目录。
