...
收到来自电话号码的 Telegram 通知后，直接**引用该通知**并输入文字，SMS Bridge 会让本机「信息」App 以 SMS 回复原号码。每条通知只能回复一次，路由信息仅保存 7 天且不包含短信原文；引用过期通知、测试通知或 iMessage 邮箱通知不会发送。

首次发送时，macOS 可能要求允许 SMS Bridge 自动化控制「信息」。请仅在该提示明确指向你启动的 SMS Bridge/Python 时允许。此功能仅接受已配对 Telegram 私聊中的引用回复，单条最多 1000 个字符；Discord Webhook 仍只支持接收通知。
